...
class SwittiPipeline:
    ...
    @staticmethod
    def _resolve_device(device: str | torch.device | None):
        if device is None:
            return "cuda" if torch.cuda.is_available() else "cpu"
        return str(device)

    @staticmethod
    def _resolve_dtype(device: str, dtype: torch.dtype):
    ...
    def from_pretrained(cls,
        ...
        torch_dtype = cls._resolve_dtype(device, torch_dtype)
    ...
    def __call__(
        ...
        use_cond_fast_path = (
            cond_context is not None
            and condition_vector is not None
            and cfg == 0.0
            and turn_off_cfg_start_si == 0
        )

        if use_cond_fast_path:
            context = cond_context
            cond_vector = condition_vector
            B = context.shape[0]
            if cond_attn_bias is None:
                context_attn_bias = torch.zeros(
                    B,
                    context.shape[1],
                ...
                context_attn_bias = cond_attn_bias
        else:
            context, cond_vector, context_attn_bias = self.encode_prompt(prompt, null_prompt)
            B = context.shape[0] // 2

        def _match_len(tensor: torch.Tensor, target_len: int, pad_value: float = 0.0):
            current_len = tensor.shape[1]
            if current_len == target_len:
                return tensor
            if current_len > target_len:
                return tensor[:, :target_len]
            pad_shape = (tensor.shape[0], target_len - current_len, tensor.shape[2])
            pad = torch.full(pad_shape, pad_value, device=tensor.device, dtype=tensor.dtype)
            return torch.cat([tensor, pad], dim=1)

        def _match_bias_len(tensor: torch.Tensor, target_len: int, pad_value: float):
            current_len = tensor.shape[1]
            if current_len == target_len:
                return tensor
            if current_len > target_len:
                return tensor[:, :target_len]
            pad_shape = (tensor.shape[0], target_len - current_len)
            pad = torch.full(pad_shape, pad_value, device=tensor.device, dtype=tensor.dtype)
            return torch.cat([tensor, pad], dim=1)

        if cond_context is not None and not use_cond_fast_path:
            uncond_context = context[B:]
            cond_len = cond_context.shape[1]
            uncond_context = _match_len(uncond_context, cond_len, pad_value=0.0)
            context = torch.cat([cond_context, uncond_context], dim=0)

            uncond_attn_bias = context_attn_bias[B:]
            if cond_attn_bias is None:
                cond_attn_bias = torch.zeros(
                    B,
                    cond_len,
                    device=context_attn_bias.device,
                    dtype=context_attn_bias.dtype,
                )
            uncond_attn_bias = _match_bias_len(uncond_attn_bias, cond_len, pad_value=-float("inf"))
            context_attn_bias = torch.cat([cond_attn_bias, uncond_attn_bias], dim=0)
        if condition_vector is not None and not use_cond_fast_path:
            cond_vector[:B] = condition_vector# 2,1280


        cond_vector = switti.text_pooler(cond_vector)

        batch_factor = B if use_cond_fast_path else (2 * B)

        if switti.use_crop_cond:
            crop_coords = get_crop_condition(batch_factor * [TRAIN_IMAGE_SIZE[0]],
                                             batch_factor * [TRAIN_IMAGE_SIZE[1]],
                                             ).to(cond_vector.device)
            crop_embed = switti.crop_embed(crop_coords.view(-1)).reshape(batch_factor, switti.D)
        ...
        next_token_map = (
            sos.unsqueeze(1)
            + switti.pos_start.expand(batch_factor, switti.first_l, -1)
        ...
        for si, pn in enumerate(switti.patch_nums):  # si: i-th segment
            ...
            if (not use_cond_fast_path) and si >= turn_off_cfg_start_si:
                apply_smooth = False
                x_BLC = x_BLC[:B]
                context = context[:B]
            ...
            # Guidance
            if use_cond_fast_path:
                if last_scale_temp is not None:
